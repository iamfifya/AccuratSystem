...
* **Высокопроизводительный десктопный терминал** администратора/кассира (WPF .NET Framework 10, частичный паттерн MVVM).
...
* **`AccuratPanelCarWashing/`** — Десктопный клиент (WPF). Более не используется и не обновляется, оставлен как исторический артефакт.
* **`AccuratPanelCWD/`** — Десктопный клиент (WPF). Основное рабочее место кассира-администратора. Управление живой очередью боксов, проводка кассовых операций, расчет апселл-бонусов, синхронизация по WebSockets (SignalR) и локальный экспорт аналитики.
...
## 🚀 Roadmap (УСТАРЕЛО) (далее отслеживается в GitHub Issues)
...
* **Desktop Client:** C# latest, WPF (.NET Framework 10), LiveCharts, ClosedXML, Newtonsoft.Json, SignalR Client.
